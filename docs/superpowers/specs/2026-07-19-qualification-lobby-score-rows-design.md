...
The lobby detail groups attempts by beatmap as it does today. Each group renders one shared `MultiplayerScore` component whose composition is:

1. the existing `Beatmap` component populated with map metadata and its tournament mod;
2. one reusable `PlayerMultiplayerScore` component per player result.

Both components belong to the shared component library, not the qualification domain. `MultiplayerScore` receives generic beatmap data and an array of generic player results and owns their composition. `PlayerMultiplayerScore` owns presentation of one player's result: osu! id and name, mods, combo, accuracy, score, hit statistics, and rank. It renders an osu! avatar instead of the country flag, followed by the result summary matching the supplied reference. Neither component imports qualification DTOs or fetches data.

The lobby card remains responsible only for grouping qualification attempts and adapting them with mappool metadata into `MultiplayerScore` props. Mappool beatmap data already loaded by the tournament page is passed through the schedule and lobby detail components; no duplicate beatmap request is added.
...
The existing `Beatmap` component keeps its current banner, metadata, links, copy actions, and tournament mod badge. Each shared `PlayerMultiplayerScore` row visually follows the reference:
...
- Focused frontend tests cover generic `MultiplayerScore` composition and `PlayerMultiplayerScore` formatting without qualification-domain imports.
