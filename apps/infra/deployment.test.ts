...
describe("production Docker images", () => {
  for (const app of ["backend", "frontend"]) {
    it(`${app} has a multi-stage image with healthcheck`, () => {
      ...
      expect(file(path).match(/^FROM /gm)?.length ?? 0).toBeGreaterThanOrEqual(
        2,
      );
...
describe("production Compose services", () => {
  ...
  it("loads VPS-owned env files and exposes application ports", () => {
    ...
    expect(compose).toContain('"${BACKEND_PORT:-3000}:3000"');
    expect(compose).toContain('"${FRONTEND_PORT:-5173}:3000"');
  });
});
...
describe("independent deployment workflows", () => {
  ...
  it("deploys only backend for backend paths", () => {
    ...
    expect(workflow).toContain("build backend migrate");
    expect(workflow).toContain("run --rm --no-deps migrate");
    expect(workflow).toContain(
      "up -d --no-deps --wait --wait-timeout 120 backend",
    );
  });
  ...
  for (const path of [backendPath, frontendPath]) {
    ...
    it(`${path} prunes dangling images after deployment`, () => {
      const workflow = file(path);
      const deploy = workflow.lastIndexOf("up -d ");
