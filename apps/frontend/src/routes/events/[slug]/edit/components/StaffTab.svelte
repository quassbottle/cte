		...
		<div class="rounded-md border border-border p-4">
			...
			<div class="mb-3 flex flex-wrap gap-2">
				{#each role.members as member (member.id)}
					<form
						method="post"
						action="?/removeTournamentStaff"
						use:enhance
						class="flex items-center gap-2 rounded border px-2 py-1 text-sm"
					>
						<input type="hidden" name="roleId" value={role.id} />
						<input type="hidden" name="userId" value={member.id} />
						<span>{member.osuUsername}</span><Button type="submit" size="sm" variant="ghost"
							>Remove</Button
						>
					</form>
				{/each}
			...
			{#if form && !form.ok && form.roleId === role.id}<p class="mt-2 text-sm text-destructive">
					{form.message}
				</p>{/if}
		</div>
	{/each}
